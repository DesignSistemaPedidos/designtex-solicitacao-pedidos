...
def conectar_postgresql():
    """Conectar ao PostgreSQL (local ou Railway) com tratamento robusto"""
        ...
        for encoding in encodings_para_testar:
            try:
                ...
                if 'codec' in error_msg or 'decode' in error_msg:
                    print(
                        f"   ❌ Erro de encoding {encoding}: {error_msg[:60]}...")
                else:
                    print(f"   ❌ Erro de conexão: {error_msg[:80]}...")
                ...
                print(f"   ❌ Erro PostgreSQL {encoding}: {str(e)[:60]}...")
                if 'conn' in locals():
                    conn.close()
                continue

            except Exception as e:
                print(f"   ❌ Erro geral {encoding}: {str(e)[:60]}...")
                if 'conn' in locals():
                    conn.close()
                continue
...
def criar_pedido(dados_pedido):
    """Criar novo pedido no PostgreSQL"""
    ...
    try:
        ...
        """, (
            ...
            dados_pedido.get('valor_total', 0.0)
        ))
...
def buscar_pedidos():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("""
            SELECT p.id, p.numero_pedido, p.cnpj_cliente, c.razao_social, 
...
<head>
    ...
    <title>DESIGNTEX TECIDOS - Sistema Completo</title>
    <style>
...
<body>
    <div class="container">
        ...
        <p class="subtitle">Sistema Completo de Pedidos - PostgreSQL</p>
        ...
        <div style="margin: 30px 0;">
            <a href="/criar-pedido" class="btn btn-primary">🆕 CRIAR PEDIDO</a>
            <br>
            <a href="/pedidos-lista" class="btn">📋 Ver Pedidos</a>
        ...
        <!-- LINKS TÉCNICOS -->
        <div>
            <a href="/health" class="btn" style="background: #17a2b8;">🔍 Health Check</a>
        </div>
        
        <div class="info">
            <h3>📋 Endpoints da API:</h3>
            <ul class="endpoints">
                <li><code>GET /pedidos</code> - Lista de pedidos (JSON)</li>
        ...
        <div class="info">
            ...
            <code style="background: #2d5f2d; color: white; padding: 8px; border-radius: 4px; display: block; margin-top: 10px;">
                http://127.0.0.1:5001/pedidos<br>
                http://127.0.0.1:5001/clientes<br>

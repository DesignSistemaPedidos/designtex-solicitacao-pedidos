...
EMAIL_CONFIG = {
    ...
    'smtp_port': 587,
    'email': 'samuel.mc85@gmail.com',  # Configure aqui
    'password': 'wvec ktnj etnf qzzi',     # Configure aqui
...
def gerar_numero_pedido():
    """Gerar número sequencial DTX-AAAAMMDD-0001"""
    ...
    cursor.execute(
        'SELECT ultimo_numero FROM pedidos_sequencia WHERE data = ?', (data_hoje,))
    resultado = cursor.fetchone()

    if resultado:
        proximo_numero = resultado[0] + 1
        cursor.execute('UPDATE pedidos_sequencia SET ultimo_numero = ? WHERE data = ?',
                       (proximo_numero, data_hoje))
    else:
        proximo_numero = 1
        cursor.execute('INSERT INTO pedidos_sequencia (data, ultimo_numero) VALUES (?, ?)',
                       (data_hoje, proximo_numero))

    conn.commit()
    conn.close()

    return f"DTX-{data_hoje}-{proximo_numero:04d}"
...
def submit_pedido():
    """Submissão com download de PDF e envio de email"""
    try:
        ...
        return jsonify({
            ...
            'arquivo': filename,
            'pdf': pdf_filename,
            'pdf_url': f'/download_pdf/{pdf_filename}',  # URL para download
...
def gerar_pdf_pedido(dados, pdf_path):
    """Gerar PDF com logo personalizada"""
    try:
        ...
        tem_ret = False
        if ('RET' in dados['tabela_precos']['icms_normal'] or
            'RET' in dados['tabela_precos']['icms_ld'] or
                dados['dados_corpo']['regime_ret'] == 'Sim'):
            tem_ret = True
...
def health_check():
    ...
    return jsonify({
        ...
        'timestamp': datetime.now().strftime('%d/%m/%Y %H:%M:%S'),
        'empresa': 'Designtex Tecidos',
        'versao': '2.2 - Email + Download + Numeração Sequencial',
        'recursos': [
            'Numeração sequencial DTX-AAAAMMDD-0001',
            'Download automático de PDF',
            'Envio de email automático',
            'Logo personalizada no PDF',
            'R.E.T (Somente MG)'
        ]
    })
...
if __name__ == '__main__':
    ...
    print("🚀 Iniciando DESIGNTEX TECIDOS - Versão Completa")
    print("=" * 70)
    ...
    print("🔢 Numeração: DTX-AAAAMMDD-0001 (sequencial)")
    print("🖼️  Logo: Coloque 'logo_designtex.png' na pasta 'static'")
    print("⚠️  Configure EMAIL_CONFIG no código")
    print("=" * 70)
